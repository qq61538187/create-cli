...
import { getRegistries , readFile, writeFile,deleteFileSync, printMessages, printSuccess} from "./helpers"
...
export const onList = async  ()=>{
 ...
 printMessages( list.map((item:Repo,index:number)=>`[${index}]${item.name} ---------- ${item.url} ---------- clone: ${item.clone} ----------describe:${item.describe||''}`))
}
export const onAdd = async (name:string, url:string, clone:string, describe:string)=>{
